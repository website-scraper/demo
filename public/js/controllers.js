...
var module = angular.module('webScraperControllers', []);

module.controller('IndexCtrl', function ($scope, $http, $location) {
	$scope.userAgents = [
		{
	...
	$scope.add = function () {
		$http.post('/sites', $scope.data).success(function (response) {
			$location.path('/view/' + encodeURIComponent(response.filename));
		});
	};
	...
	$scope.setDefaults = function () {
		var defaults = { url: 'http://example.com/' };
...
module.controller('ViewCtrl', function ($scope, $http, $location, $routeParams) {
	$scope.filename = '/files/' + $routeParams.filename;
});

module.controller('ListCtrl', function ($scope, $http, $location) {
